...
class Pound(Currency):
    ...
    def __str__(self):
        if self.value > 1:
            return str(self.value) + " Pounds"
        else:
            return str(self.value) + " Pound"
...
class Bank:
    def __init__(self,name,unit,value=0):
        self.name = name
        self.unit= unit #this is a class name
        self.current_account = unit(value)#this shall be an instance of currency class
    ...
    def deposit(self,currencytype):
        if isinstance(currencytype, self.unit):
            result = "successful deposit"
            self.current_account.value+=self.current_account.value
            return(result)
        else :
